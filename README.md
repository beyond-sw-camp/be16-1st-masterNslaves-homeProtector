...
- 김건동
  - ERD설계하는 프로젝트를 팀원들과 진행하면서 관계성을 확실하게 이해하지 못하고 자주 흔들렸었다. 이후로 설계할때 외래키가 잘못걸려있는 부분도 많았고 최종적으로 설계가 끝났음에도 좋아요같은 테이블이 문제가 된다는것을 깨달았다. 프론트엔드에서 만드는 모델링 작업을 생각하고 ERD와 요구사항명세서를 생각하다보니 너무 헷갈렸지만 팀원들과 같이 머리를 맞대면서 열심히 했던것 같다. DB프로젝트지만 기획부터 문서화까지 중요하다는 것을 배웠고 ERD는 스프링하면서 실력을 점차 증진해질 것 같다.
- 김찬진
  - 전월세 사기 방지라는 현실적인 문제를 다루며, 단순한 테이블 설계를 넘어 사람들의 경험을 담아낼 수 있는 데이터베이스를 고민하게 되었다. ERD에서 테이블 간의 관계성을 자연스럽게 표현하는 것이 특히 어려웠지만, 그 과정을 통해 데이터 구조가 서비스 흐름과 얼마나 밀접하게 연결되는지를 깊이 깨달을 수 있었다.  팀원들과 밤늦게까지 머리를 맞대며 끊임없이 수정하고 테스트한 끝에, 완벽하진 않더라도 우리가 만든 결과물에 대한 뿌듯함을 느낄 수 있었다.
- 김진호
  - 이번 프로젝트를 진행하며 DB의 전반적인 이해도를 높일 수 있었습니다. 특히 이론으로만 접하던 것들을 구체적으로 구현해 볼 수 있는 좋은 기회였던 것 같습니다. 아직 프로그래밍을 배우지 못해 ERD 물리 설계가 많이 미흡했으나, 그것마저도 좋은 양분이 되었던 것 같습니다. 강사님의 피드백과 저 스스로의 경험, 팀과의 협업을 바탕으로 어떤 점을 개선해 나가야 할 지, 어떤 부분을 중점적으로 공부해야 할 지 구체화 할 수 있었습니다. 또한 제 부족한 점에 대해 굉장히 직접적으로 맞닥뜨릴 수 있었던 기회였습니다. 많이 익혀야 하지 않을까 하는 반성을 하게 되었고, 스스로 개발에 대한 기준을 세울 수 있는 좋은 프로젝트였던 것 같습니다.
- 이우영
  - 어언 3주간의 시간이 눈 깜짝할 새에 흘러가고, 드디어 팀 프로젝트의 ERD 설계 단계에 돌입하였다.
우리 팀은 "부동산 전월세 사기 방지를 위한 정보 공유 플랫폼"을 만들기로 하였다. 하지만 프로젝트를 시작하자마자 현실의 벽에 부딪혔다. 과거에 공인중개사 강사님의 강의를 2년간 촬영하는 아르바이트를 했음에도, 부동산 관련 내용은 머릿속에서 거의 증발한 상태였고, 사기 유형이나 경매 절차 등에 대한 기본 개념조차 생소하게 느껴졌다. 이로 인해 "어떤 정보를, 어떤 방식으로 제공해야 사기를 방지할 수 있을까?" 하는 질문이 프로젝트 초반 내내 머리를 떠나지 않았다. 특히 어려웠던 점은 다음과 같았다: ERD 설계에서 필요한 개념 자체가 명확하지 않았고, 프론트엔드와 백엔드의 역할 및 프로세스 흐름에 대한 이해도 부족했다. 그 결과, 무엇을 중심으로 설계를 해야 하는지, 그리고 어떤 기능이 데이터베이스 구조에 어떤 영향을 미치는지를 판단하는 것이 매우 어렵게 느껴졌다. 그럼에도 이번 프로젝트를 통해 단순히 기능 중심이 아닌, 사용자 관점에서 어떤 정보가 필요한지, 관리자 입장에서는 어떤 데이터를 다뤄야 하는지를 함께 고민하게 되었고, 이를 통해 데이터베이스를 보다 효율적으로 설계하는 감이 조금은 잡힌 것 같다는 생각이 든다.
